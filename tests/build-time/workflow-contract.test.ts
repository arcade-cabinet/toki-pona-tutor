...
describe('GitHub Actions run/release/deploy contract', () => {
    ...
    it('keeps ci.yml as the PR gate with reviewer artifacts after validation', () => {
        ...
        expect(ci).toContain('CAPACITOR: \'true\'');
    ...
    it('keeps release.yml on release-please PAT plus versioned workflow artifacts', () => {
        ...
        expect(release).toContain('GITHUB_PAGES: \'true\'');
        expect(release).toContain('node scripts/deploy/verify-pages-bundle.mjs dist');
        expect(release).toContain('CAPACITOR: \'true\'');
        expect(release).toContain('name: web-bundle-${{ needs.release-please.outputs.tag_name }}');
        expect(release).toContain('name: android-debug-apk-${{ needs.release-please.outputs.tag_name }}');
    ...
    it('keeps cd.yml consuming completed release workflow artifacts, not release events', () => {
        ...
        expect(cd).toContain('for required in tag version sha web_artifact web_file apk_artifact apk_file');
    ...
    it('keeps every workflow action pinned to a full commit SHA', () => {
        for (const name of workflowNames()) {
            ...
            const usesLines = source.matchAll(/^\s*uses:\s*([^@\s#]+)@([^\s#]+)(?:\s+#\s*(.+))?$/gm);

            for (const match of usesLines) {
                const [, action, ref, comment] = match;
                expect(ref, `${name}: ${action}@${ref} must be pinned to a 40-char SHA`).toMatch(/^[a-f0-9]{40}$/);
                expect(comment, `${name}: ${action}@${ref} should keep a version comment for update review`).toMatch(/^v\d+\./);
            }
        }
    });
});

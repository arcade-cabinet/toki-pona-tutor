...
import { typeMultiplier, TP_TYPES, type TpType } from '../../src/modules/main/type-matchup';
...
describe('typeMultiplier — 5×5 matrix per ARCHITECTURE §Types', () => {
    ...
    it('covers all 25 primary-type matchups without throwing', () => {
        ...
        for (const a of TP_TYPES) {
            for (const d of TP_TYPES) {
                const key = `${a}>${d}`;
                seen[key] = typeMultiplier(a as TpType, d as TpType, false);
            }
        }

...
describe("buildChunkParsedMap", () => {
    ...
    it("returns a valid parsedMap for origin outdoor chunk", () => {
    ...
    it("all tile GIDs are positive integers", () => {
        ...
        expect(data.every((gid) => Number.isInteger(gid) && gid > 0)).toBe(true);
    });
    ...
    it("has an object group layer with edge warp objects", () => {
        ...
        const warps = objLayer!.objects?.filter((o) => o.type === "edge_warp") ?? [];
    ...
    it("north warp targets chunk_0_-1", () => {
        ...
        const northWarp = objLayer.objects?.find(
            (o) => o.type === "edge_warp" && o.properties?.find((p: {name: string}) => p.name === "direction")?.value === "north",
        );

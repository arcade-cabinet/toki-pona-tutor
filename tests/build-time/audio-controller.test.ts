...
describe('publicAssetPath', () => {
    it('preserves local dev root base', () => {
        expect(publicAssetPath('/rpg/audio/bgm-village.ogg', '/')).toBe('/rpg/audio/bgm-village.ogg');
    });

    it('prefixes GitHub Pages project base', () => {
        expect(publicAssetPath('/rpg/audio/bgm-village.ogg', '/poki-soweli/'))
            .toBe('/poki-soweli/rpg/audio/bgm-village.ogg');
    });
...
describe('getPlayableSound', () => {
    it('returns already-playable sounds unchanged', async () => {
        ...
        await expect(getPlayableSound({ getSound: () => sound }, 'bgm_village')).resolves.toBe(sound);
    });
    ...
    it('registers raw resolver definitions through the backend addSound hook', async () => {
        ...
        }, 'bgm_village')).resolves.toBe(registered);
    });
    ...
    it('ignores raw resolver definitions when the backend cannot register them', async () => {
        ...
        }, 'bgm_village')).resolves.toBeUndefined();
    });
});
...
describe('BgmCrossfadeController', () => {
    ...
    it('cross-fades and stops the previous track after the fade window', async () => {
        ...
        expect(sounds.get('bgm_forest')!.calls).toContain(`fade:0:0.6:${BGM_CROSSFADE_MS}:bgm_forest:play`);
        expect(sounds.get('bgm_village')!.calls).toContain(`fade:0.7:0:${BGM_CROSSFADE_MS}:bgm_village:play`);

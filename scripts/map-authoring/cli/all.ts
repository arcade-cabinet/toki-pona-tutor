...
async function main(): Promise<void> {
    ...
    const specs = allSpecs
        ? (await readdir(specsDir)).filter((f) => f.endsWith(".ts")).map((f) => basename(f, ".ts"))
        : requestedSpecs;
    if (specs.length === 0) {
        console.error(
            "usage: pnpm author:all <map-id> [<map-id>...] [--dry-run] | --all [--dry-run]",
        );
        process.exit(1);
    }
...
async function processOne(id: string, options: { dryRun: boolean }): Promise<void> {
    ...
    console.log(
        `  ✓ ${options.dryRun ? "would render" : "rendered"} ${basename(pngPath)} (${png.width}×${png.height})`,
    );
}

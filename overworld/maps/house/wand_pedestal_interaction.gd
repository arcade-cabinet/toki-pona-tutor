...
## Keep track of the id of the item currently placed on the pedestal, from OpenRpgInventory.ItemTypes enum.
...
# Argument is expected to be the key (item type) of the OpenRpgInventory.ItemTypes enum.
func _on_dialogic_signal_event(argument: String) -> void:
	# Convert the argument into an item id, as defined by the OpenRpgInventory.ItemTypes enum.
	var item_id: = OpenRpgInventory.ItemTypes.get(argument.to_upper(), -1) as int
	if not item_id in VALID_ITEMS:
		return
	
	# Convert the value specified by pedestal_requirement to an item id, as defined by the
	# OpenRpgInventory.ItemTypes enum.
	var expected_wand: = pedestal_requirement.to_upper() + "_WAND"
	var expected_wand_id: = OpenRpgInventory.ItemTypes.get(expected_wand, -1) as int
	
	# The pedestals will track which item is on them via the item ID. This is the value of the
	# OpenRpgInventory.ItemTypes enum (which corresponds to the key, which is a string such as "Coin").
	# Note that an argument that is not found within OpenRpgInventory.ItemTypes will return -1, an invalid
	# value that will trigger the "remove item from pedestal" condition.
	if item_id < 0:
		if current_item_id in OpenRpgInventory.ItemTypes.values():
			OpenRpgInventory.restore().add(current_item_id)
	
	else:
		OpenRpgInventory.restore().remove(item_id)
	
	# The item_id shows what is currently on the pedestal. Set the sprite's texture to match.
	current_item_id = item_id
	_sprite.texture = OpenRpgInventory.get_item_icon(item_id)

...
-   Choices are tappable rows. The local `rpg-dialog.ce` exposes stable `data-testid="dialog-choice-{n}"` hooks, keeps `data-choice-index` for compatibility, and brand CSS gives every row a readable parchment button treatment with a 44 px coarse-pointer floor.
...
-   The dialog GUI keeps stable `data-testid="dialog-choice-{n}"` hooks for tap/click automation, with `data-choice-index="{n}"` retained only as compatibility metadata.
...
| Dialog choice N              | `data-testid="dialog-choice-{n}"`                                                   |

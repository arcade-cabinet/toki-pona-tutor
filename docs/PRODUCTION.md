...
-   Latest verified release: `v0.30.0` (carries the content-depth pass: dossier scaffold, per-region NPC+sign dossiers, multi-state dialog selector, dossier NPC runtime spawning, per-NPC graphics, post-clear world states, Tiled sign objects, integration coverage; release-please landed v0.4.0 → v0.30.0 through the same pipeline).
...
| Story and quest journey     | strong  | story bible + 15 quests, 81 dossier NPCs / 152 dialog nodes / 35 signs across 7 regions, multi-state reactive dialogue with post-clear "world heals" coverage on every non-trivial NPC, dossier NPC runtime with required_flag gating + flag-aware selector, post-clear green-dragon re-fight, scripted opening scene | quest-chain flag wiring for dossier-gated states (snowbird, lost-hiker, torch-path, shopkeep, cold-hands), NPC arc polish, optional side paths |

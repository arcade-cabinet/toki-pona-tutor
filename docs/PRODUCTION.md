...
-   Latest verified release: `v0.34.0` (carries the full content-depth pass through T74: region dossiers, multi-state dialog selector, dossier NPC runtime with per-NPC graphics and required_flag gating, post-clear world states for every non-trivial NPC, Tiled sign objects, dialog on_exit.set_flag triggers, 4 active quest turn-ins, shopkeep_first_sale and proofs_all_four flag wiring; release-please landed v0.4.0 → v0.34.0 through the same pipeline).
...
| Story and quest journey     | strong  | story bible + 15 quests, 81 dossier NPCs / 156 dialog nodes / 35 signs across 7 regions, multi-state reactive dialogue with post-clear "world heals" coverage on every non-trivial NPC, dossier NPC runtime with required_flag gating + flag-aware selector, 4 active quest turn-ins wired via dialog on_exit, all dialog-referenced flags have setters (badges, quest turn-ins, shopkeep_first_sale, proofs_all_four), post-clear green-dragon re-fight, scripted opening scene | visual hiding of pre-gate NPCs, deeper quest chains, 3-state NPC expansion to 4 states, optional side paths |

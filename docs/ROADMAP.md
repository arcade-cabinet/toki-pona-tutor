...
| 11    | Onboarding + Opening Scene | Story is the asset that lasts — close the landing→gameplay cliff and script the first 30 seconds |   6 |   1 |   4 |
...
| ID     | Task                                                                      | Status | Notes                                                                                                                         |
| ------ | ------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------- |
| T11-01  | Close the landing→gameplay quality cliff                                  | 🟡     | Meta row — T11-05/06/07/08/09/11 shipped and verified; T11-02/03/04/10 reopened after live visual audit found their PRs didn't actually fix the runtime issue. Still owes: player differentiation, real black-rect fix, brown-square root cause, non-text-component first-play cue. |
| T11-02  | Distinguish the player sprite from NPCs                                   | ⬜     | **REOPENED after live visual audit**: PR #136's Components.text approach produces unrenderable output in RPG.js v5 beta (shader/source splatter on canvas). Module `src/modules/main/player-identity.ts` still exists but the `applyPlayerNameTag` calls are disabled in `src/modules/main/player.ts`. Need a non-Components.text differentiation (direction indicator, unique sprite, outline shader). |
| T11-03  | Fix black rectangle placeholders on `riverside_home`                      | ⬜     | **REOPENED after live visual audit**: PR #138's tree placements were reverted in `scripts/map-authoring/specs/riverside_home.ts` because the Trees_Seasons atlas has 97×124 sprites (vs 16-px grid), causing them to clip onto the road and villagers. The original "black rectangles" reported in the onboarding capture are **still present in runtime** and have a different root cause (unknown — not trees, not NPCs, not painted tiles). Needs live browser diagnosis. |
| T11-04  | Label or remove the unexplained brown square in the starter village      | ⬜     | **REOPENED after live visual audit**: PR #142 correctly removed the `[2, 7, 4, 2]` dirt plot, but the actual brown square visible in runtime is a different artifact (80×80 = one tile at 5x scale, located at tile coord ~(8, 7)). Root cause unknown — possibly a miskeyed transition tile or a stray decoration rendering wrong. Needs live browser diagnosis. |

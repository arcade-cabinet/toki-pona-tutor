...
`pnpm test:e2e:full` is local. The full suite currently has 29 full browser tests across 17 files and covers title/save flows, settings, text speed, clue overlay, mobile HUD/menu/status/hint/tap controls, party/bestiary, side quests, wild battle, action-battle movebar, visual audit, asset-load audit against the Pages preview, onboarding capture, and the golden path to credits.

...
src/
  ...
  modules/
    main/
      ...
      combat-visuals.ts     — configured combat defeat/feedback visual parameters
      ...
      green-dragon.ts       — endgame encounter; gated on all four region badges
...
-   **kasi** beats **telo**
-   **lete** (ice) specialist cold coverage; weak offensive coverage otherwise
...
**Set-piece:** hand-placed fights — rivals (jan Ike), jan lawa (jan Telo / jan Wawa / jan Lete / jan Suli), and the green dragon at endgame.
...
| Combat chrome                 | `src/config/provide-combat-chrome.ts`, `src/config/poki-combat-hp-bar.ce`, `src/config/poki-combat-feedback.ce`, `src/config/poki-combat-target-reticle.ce`, `src/config/provide-lead-movebar-gui.ts`, `src/config/poki-lead-movebar.ce`, `src/config/provide-wild-battle-gui.ts`, `src/config/poki-wild-battle.ce`, `src/modules/main/combat-chrome.ts`, `src/modules/main/combat-visuals.ts`, `src/modules/main/wild-battle-view.ts`, and combat CSS in `src/styles/brand.css` |
...
-   **Boss vs creature tiering by animation depth** — animated sprites (`bosses/`); static sprites (`creatures/`). Green dragon is the designated endgame set-piece.
